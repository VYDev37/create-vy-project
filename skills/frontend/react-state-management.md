...
Comprehensive guidelines for managing state across modern React and Next.js applications with clarity, type safety, and minimal re-renders.
...
| **Server / Async State** | Server data & caching | Next.js Server Components / React Query | User profiles, database queries, external APIs |
| **Form State** | Form inputs & validation | **React Hook Form + Zod** | Login, Registration, multi-step wizards |
| **URL State** | Shareable filters / pagination | Next.js `useSearchParams` | Table pagination, search queries, active tabs |
...
## 4. Server State in Next.js App Router

In Next.js App Router, prefer fetching data directly in **Server Components** instead of fetching inside `useEffect` and storing in client state:
...
- ❌ **No God Stores**: Do not create a single giant Zustand store for the entire app. Create focused stores (e.g. `AuthStore.ts`, `UiStore.ts`, `CartStore.ts`).
- ❌ **No Duplicated Server State**: Do not copy server database queries into Zustand if a Server Component or simple query hook can provide it directly.
- ❌ **No Prop Drilling Beyond 2 Levels**: If passing props down more than 2-3 levels without intermediate consumption, use a lightweight store or Context.
