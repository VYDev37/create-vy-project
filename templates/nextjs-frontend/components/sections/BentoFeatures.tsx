...
import { Shield, Server, Layers, Cpu } from "lucide-react";
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-2xl mx-auto mb-12 space-y-3">
          ...
          <h2 className="text-3xl sm:text-4xl font-bold tracking-tight text-foreground">
            Simple, organized frontend architecture.
          </h2>
          <p className="text-sm sm:text-base text-muted-foreground">
            A clean client setup designed to communicate seamlessly with your backend API.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Card className="md:col-span-2 p-6 md:p-8 flex flex-col justify-between border-border/60 bg-gradient-to-br from-card to-muted/40 relative overflow-hidden">
            <div className="space-y-3 z-10 max-w-md">
              ...
              <h3 className="text-xl font-bold tracking-tight text-foreground">
                Go Fiber API Integration
              </h3>
              <p className="text-sm text-muted-foreground leading-relaxed">
                Pre-configured Axios client that automatically sends HTTP-only session cookies with every backend request.
              </p>
            </div>
          ...
          <Card className="p-6 md:p-8 flex flex-col justify-between border-border/60 bg-card">
            <div className="space-y-3">
              ...
              <p className="text-sm text-muted-foreground leading-relaxed">
                Works directly with Go Fiber JWT cookies without storing tokens insecurely in localStorage.
              </p>
            </div>
            <div className="mt-6 pt-4 border-t border-border/40 text-xs font-mono text-emerald-600 dark:text-emerald-400">
              Safe Cookie Sessions
            </div>
          </Card>
